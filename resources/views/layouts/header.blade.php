...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <div class="tg-header-banner">
        ...
        <div class="container">
            <a class="navbar-brand" href="{{ url('/') }}"><img src="{{ asset('assets/imgs/inline-logo-black.png') }}"
                    width="340" alt="BBC TopGear India"></a>
                    ...
                    @foreach($menu as $item)
                        <li class="nav-item dropdown">
                            <a class="nav-link {{ $item->submenus->isNotEmpty() || ($item->ID == 3 && $item->dynamicSubmenus->isNotEmpty()) ? 'dropdown-toggle' : '' }}"
                                ...
                                id="{{ $item->Name }}" role="button" {{ $item->submenus->isNotEmpty() || ($item->ID == 3 && $item->dynamicSubmenus->isNotEmpty()) ? 'data-bs-toggle=dropdown' : '' }}
                                aria-expanded="false">
                                {{ $item->Name }}
                            </a>
                                ...
                                <ul class="dropdown-menu" aria-labelledby="{{ $item->Name }}">
                                    @php
                                        ...
                                        if (isset($item->Name) && strtolower($item->Name) === 'news') {
                                            $carsFirst = $item->submenus->filter(function ($s) {
                                                return strtolower($s->Name) === 'cars';
                                            });
                                            $others = $item->submenus->reject(function ($s) {
                                                return strtolower($s->Name) === 'cars';
                                            });
                                                ...
                                                <ul class="dropdown-menu">
                                                    @php
                                                        ...
                                                        if (isset($submenu->Name) && strtolower($submenu->Name) === 'bikes/scooters') {
                                                            $firstRide = $submenu->reviewSubmenus->filter(function ($r) {
                                                                return strtolower($r->Name) === 'first ride';
                                                            });
                                                            $otherReviews = $submenu->reviewSubmenus->reject(function ($r) {
                                                                return strtolower($r->Name) === 'first ride';
                                                            });
                                                    ...
                                                    @foreach($reviewSubmenusOrdered as $reviewSubmenu)
                                                        <li>
                                                            <a class="dropdown-item"
                                                                href="{{ url('reviews/' . $submenu->Code . '/' . $reviewSubmenu->Code) }}">
                                                                {{ $reviewSubmenu->Name }}
                                                            </a>
                                                        </li>
                                                    @endforeach
                ...
                <div class="nav-search">
                    <button class="tg-search">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" stroke="currentColor"
                            fill="currentColor" stroke-width="0" width="20" height="20" data-testid="SvgGelIconSearch">
                            <path
