            ...
            <form action="{{ route('adminfeatures.update', $feature->FeatureID) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Featured Image -->
                        <div class="space-y-2">
                            <label for="Thumbimage" class="block text-sm font-medium text-gray-700">Upload Featured Image <span class="text-gray-500 text-sm">(Min: 1700 x 950, Max: 3.5MB)</span></label>
                        ...
                        <!-- Slider Images -->
                        <div class="space-y-2">
                            <label for="Image" class="block text-sm font-medium text-gray-700">Slider Images (Upload multiple) <span class="text-gray-500 text-sm">(Min: 1400 x 800, Max: 3.5MB per image)</span></label>
                            ...
                            @if($feature->images->count() > 0)
                                <div class="mt-4">
                                    ...
                                    <div id="sortableImages" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                        @foreach ($feature->images->sortBy('DisplayOrder') as $index => $image)
                                            <div class="image-item bg-white border-2 border-gray-200 rounded-lg p-2 shadow-sm hover:shadow-md transition-shadow cursor-move" data-image-id="{{ $image->FeaturesImageID }}">
                                                <div class="relative">
                                                    ...
                                                    <button type="button" class="absolute top-1 right-1 bg-red-500 text-white text-xs px-2 py-1 rounded-full hover:bg-red-600 remove-image" data-image-id="{{ $image->FeaturesImageID }}">
                                                        ×
                                                    </button>
                                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateImageOrder() {
        ...
        fetch('{{ route("adminfeatures.updateImageOrder") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
