<x-app-layout>
    ...
    <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            ...
            <form method="GET" action="{{ route('admineditorials.index') }}" class="flex items-center mb-4 space-x-4">
                <input type="text" name="search" placeholder="Search Editorials..." value="{{ request('search') }}" class="mr-2 px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div class="flex justify-between mb-4">
                <h3 class="text-lg leading-6 font-medium text-gray-900">All Editorials</h3>
                <a href="{{ route('admineditorials.create') }}" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">
                    Create New Editorial Post
                </a>
            </div>
                            ...
                            <tr class="border-b hover:bg-gray-50 transition duration-200 {{ $loop->odd ? 'bg-gray-50' : 'bg-white' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <span class="flex items-center">
                                        ...
                                        {{ \Carbon\Carbon::parse($editorial->PublishDate)->format('M d, Y') }}
                                    </span>
                                </td>

                                <!-- Status -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if ($editorial->IsActive)
                                ...
                                <!-- Actions -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="{{ route('admineditorials.edit', $editorial->EditorialID) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                    <form action="{{ route('admineditorials.destroy', $editorial->EditorialID) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
