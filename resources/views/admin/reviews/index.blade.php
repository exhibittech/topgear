...
<x-app-layout>
    ...
    <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            ...
            <form method="GET" action="{{ route('adminreviews.index') }}" class="flex items-center mb-4 space-x-4">
                <input type="text" name="search" placeholder="Search Reviews..." value="{{ request('search') }}" class="mr-2 px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div class="flex justify-between mb-4">
                <h3 class="text-lg leading-6 font-medium text-gray-900">All Reviews</h3>
                <a href="{{ route('adminreviews.create') }}" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">
                    Create New Review
                </a>
            </div>
                ...
                <table class="min-w-full table-auto border-collapse bg-white shadow-md rounded-lg">
                    <thead>
                        <tr class="bg-gray-100 border-b">
                            <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase">Review Title</th>
                            ...
                            <tr class="border-b hover:bg-gray-50 transition duration-200 {{ $loop->odd ? 'bg-gray-50' : 'bg-white' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                                        {{ $review->category->Name ?? 'No Category' }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <span class="flex items-center">
                                        ...
                                        {{ \Carbon\Carbon::parse($review->PublishDate)->format('M d, Y') }}
                                    </span>
                                </td>
                                ...
                                <!-- Actions -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="{{ route('adminreviews.edit', $review->ReviewsID) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                    <form action="{{ route('adminreviews.destroy', $review->ReviewsID) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
