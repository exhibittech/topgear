<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow sm:rounded-lg p-6">
            ...
            <form action="{{ route('adminnews.update', $news->NewsID) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="Name" value="{{ old('Name', $news->Name) }}" class="block w-full px-3 py-2 border @error('Name') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" id="Name" placeholder="Title" required>
                            @error('Name')
                        ...
                        <!-- News Category -->
                        <div class="space-y-2">
                            <label for="CategoryID" class="block text-sm font-medium text-gray-700">News Category</label>
                            <select name="CategoryID" id="CategoryID" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border @error('CategoryID') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" required>
                                @foreach ($categories as $category)
                                    <option value="{{ $category->ID }}" {{ old('CategoryID', $news->MenuID) == $category->ID ? 'selected' : '' }}>{{ $category->Name }}</option>
                                @endforeach
                            </select>
                            @error('CategoryID')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <div id="featuredImagePreview" class="mt-3">
                                @if($news->ImagePath)
                                    <img src="{{ asset($news->ImagePath) }}" alt="Featured Image" class="rounded-md shadow-md max-w-full h-auto">
                                @endif
                            </div>
                            @error('Thumbimage')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" id="description" class="block w-full px-3 py-2 border @error('description') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description', $news->Description) }}</textarea>
                        @error('description')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="MetaTitle" value="{{ old('MetaTitle', $news->MetaTitle) }}" class="block w-full px-3 py-2 border @error('MetaTitle') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" id="MetaTitle" placeholder="Meta Title">
                            @error('MetaTitle')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="Keyword" value="{{ old('Keyword', $news->Keyword) }}" class="block w-full px-3 py-2 border @error('Keyword') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" id="Keyword" placeholder="Meta Keyword">
                            @error('Keyword')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="Author" value="{{ old('Author', $news->Author) }}" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" id="Author" disabled>
                        </div>

                        <!-- Publish Date -->
                        <div class="space-y-2">
                            <label for="PublishDate" class="block text-sm font-medium text-gray-700">Publish Date</label>
                            <input type="date" name="PublishDate" value="{{ old('PublishDate', $news->PublishDate) }}" class="block w-full px-3 py-2 border @error('PublishDate') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" id="PublishDate" required>
                            @error('PublishDate')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="flex items-center">
                                <input class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" type="radio" name="IsActive" id="inlineRadio1" value="1" {{ old('IsActive', $news->IsActive) == '1' ? 'checked' : '' }}>
                                <label for="inlineRadio1" class="ml-2 block text-sm text-gray-700">Active</label>
                            </div>
                            <div class="flex items-center">
                                <input class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" type="radio" name="IsActive" id="inlineRadio2" value="0" {{ old('IsActive', $news->IsActive) == '0' ? 'checked' : '' }}>
