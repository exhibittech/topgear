                    ...
                    <div class="carousel-inner">
                        @foreach($images as $i_key => $i_value)
                            @php
                                ...
                                $altText = ucwords(trim(preg_replace('/\bElectric\b/i', '', $imageName)));
                    ...
                    <div class="carousel-indicators">
                        @foreach($images as $i_key_t => $i_val_t)
                            @php
                                ...
                                $altText = substr($altText, 0, -10);
                            @endphp
                            <button type="button" data-bs-target="#carousel-thumbnail" data-bs-slide-to="{{ $i_key_t }}"
                                class="thumbnail {{ !$i_key_t ? 'active' : '' }}" aria-current="true"
                                aria-label="Slide {{ $i_key_t + 1 }}">
                                <img src="{{ asset(($i_val_t->ImagePath) ? $i_val_t->ImagePath : $i_val_t->SliderImage) }}"
                                    class="d-block w-100" alt="{{ $altText }}">
                            </button>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="wrap-ad sticky-top">
                        <a href="https://www.exhibitstore.in/" target="_blank">
                            <img src="https://www.topgearmag.in/uploads/Banners/tgissue-jun2026.jpg" width="100%"
                                alt="TopGear Magazine June 2026" />
                        </a>
        ...
        <div class="container">
            <nav>
                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                    <button class="nav-link active" id="tg-rel-tab" data-bs-toggle="tab" data-bs-target="#nav-home"
                        type="button" role="tab" aria-controls="nav-home" aria-selected="true">Related Articles</button>
                </div>
            </nav>
                            ...
                            @foreach($relatedArticles as $related)
                                <div class="col-md-4">
                                    <div class="tgrelated-article">
                                        <div class="tg-media">
                                            <a href="{{ url('electric/' . $related->Code) }}">
                                        ...
                                        <div class="tg-reltxt">
                                            ...
                                            <span class="tg-author"><a href="{{ url('author/' . $related->Author) }}"
                                                    class="tg-review-tag">{{ $related->Author }}</a></span> -
                                            <span
                                                class="tg-article-date">{{ date('F j, Y', strtotime($related->PublishDate)) }}</span>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
